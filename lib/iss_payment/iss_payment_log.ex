defmodule StarkBank.IssPayment.Log do
  alias __MODULE__, as: Log
  alias StarkBank.Utils.Rest
  alias StarkBank.Utils.Check
  alias StarkBank.Utils.API
  alias StarkBank.IssPayment
  alias StarkBank.User.Project
  alias StarkBank.Error

  @moduledoc """
  Groups IssPayment.Log related functions
  """
  ...
  ## Attributes:
    ...
    - `:payment` [IssPayment]: IssPayment entity to which the log refers to.
    - `:errors` [list of strings]: list of errors linked to this IssPayment event.
    - `:type` [string]: type of the IssPayment event which triggered the log creation. ex: "processing" or "success"
  ...
  ## Options:
    ...
    - `:payment_ids` [list of strings, default nil]: list of IssPayment ids to filter retrieved entities. ex: ["5656565656565656", "4545454545454545"]
    - `:user` [Project]: Project struct returned from StarkBank.project(). Only necessary if default project has not been set in configs.
  ...
  def resource_maker(json) do
    %Log{
      id: json[:id],
      payment: json[:payment] |> API.from_api_json(&IssPayment.resource_maker/1),
