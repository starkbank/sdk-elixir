defmodule StarkBank.Transfer.Log do
  ...
  ## Parameters (optional):
    - `:limit` [integer]: maximum number of structs to be retrieved. Unlimited if nil. ex: 35
    - `:after` [Date or string]: date filter for structs created only after specified date. ex: ~D[2020-03-25]
    - `:before` [Date or string]: date filter for structs created only before specified date. ex: ~D[2020-03-25]
    - `:types` [list of strings]: filter retrieved structs by types. ex: "success" or "failed"
    - `:transfer_ids` [list of strings]: list of Transfer ids to filter retrieved structs. ex: ["5656565656565656", "4545454545454545"]
  ...
  ## Return:
    - list of Transfer.Log structs with updated attributes and cursor to retrieve the next page of Transfer.Log objects
  """
  @spec page(
          cursor: binary,
          limit: integer,
